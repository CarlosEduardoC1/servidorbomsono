...
exports.recuperar = async (req, res, next) => {
    ...
    if (user) {
        ...
        user.password = senha;
        console.log(user);
        await models.users.update({ password: bcrypt.hashSync(senha, 10) }, { where: { id: user.id } }).then(function (result) {
            return result;
        });
        utils.enviarEmailCliente('Recuperar Senha | Gerente WP - Ferramenta para Gerenciar seu estacionamento'
            , layoutEmail(senha), user.email);
        return res.status(200).json({ sit: 0, msg: 'E-mail enviado! Verifique sua caixa de entrada.' });

    } else {
        return res.status(200).json({ sit: 1, msg: 'E-mail não encontrado. Verifique a ortografia!' });
    }
}